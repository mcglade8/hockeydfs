...
# stacks may need to be full info from all_lines for creating the vectors
  ...
  stack2size <- 3
  testing <- F
  variance <- 0 # increase here increases lineup diversity with regards to stack selection
  darts <- 0.00 # ownership cutoff for dart throw players; lower for big slates
  num_darts <- 2 # max players below 'darts' ownership allowed
  min_own <- 0.00 # combined min own threshold
  pp_factor <- .6 # increase here increases odds of getting a PP line as a stack; likely better for larger slates
...
while(t < num_lineups){
  ...
  data <- odata %>%
    ...
    distinct()
      ...
      group_by(Team) %>%
      summarise(line_proj = sum(fpts_proj)*pp_factor, cost = sum(salary)) %>%
      mutate(stack = gsub(" ", "", paste(Team, "PP")))
      
      all_lines <- rbind(first_line, second_line, third_line, fourth_line, pp_line)
      all_lines$select <- runif(length(all_lines$Team), 0, sqrt(all_lines$line_proj)+variance)
      all_lines <- all_lines %>%
        group_by(Team) %>%
        slice_max(select)
      ...
      data<- data %>%
        mutate(stackline = case_when(
          gsub(" ", "", paste0(Team, Line)) == stacks$stack[1] | gsub(" ", "", paste0(Team, "PP")) == stacks$stack[1]~ "stack1",
          gsub(" ", "", paste0(Team, Line)) == stacks$stack[2] | gsub(" ", "", paste0(Team, "PP")) == stacks$stack[2] ~ "stack2",
...
vdarts <- as.numeric(data$DKOwnProj <= darts & data$stackline  == "unstacked" & !data$Position == "G")
...
matrix_vector <- c(vsalary, vcenter, vwing, vdef, vgoal, vutil, vplaying,  vdarts, vstack1,vstack2,  voneoff, voppg, vown)
...
const.rhs = c(50000,  2,  3,  2,   1,    8,    9, num_darts, stack1size, stack2size, 1, 0, min_own)
const.dir = c("<=", ">=",">=",">=", "==", "==", "==",  "<=", "==","==", ">=", "==", ">=")
