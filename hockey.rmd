...
# stacks may need to be full info from all_lines for creating the vectors
  ...
  stack2size <- 3
  testing <- F
  variance <- 10 # increase here increases lineup diversity with regards to stack selection
  darts <- 0.01 # ownership cutoff for dart throw players; lower for big slates
  min_own <- 0.7
  
  ### May want to add a min combined own threshhold; seems like ~1/3 of the top 30 owned are in top 1% plays; probably want min 5 of those in any lineup
...
const.rhs = c(50000,  2,  3,  2,   1,    8,    9, 1, stack1size, stack2size, 1, 0, min_own)
const.dir = c("<=", ">=",">=",">=", "==", "==", "==",  "<=", "==","==", ">=", "==", ">=")
